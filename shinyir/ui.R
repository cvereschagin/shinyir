...
page_navbar(
  ...
  sidebar = sidebar(
    dateInput("valuation_date", "Select Valuation Date", min = min(rate_data$date), max = max(rate_data$date), daysofweekdisabled = c(0,6)),
    ...
    dateInput("maturity_date", "Maturity Date", value = Sys.Date() + years(1), daysofweekdisabled = c(0,6)),
